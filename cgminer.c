...
#ifdef USE_GPIO_CS
int opt_gcs=-1; // -1 not gpio cs
#endif

#ifdef USE_ST_MCU//Extra Option
int opt_stmcu=-1; // -1 No ST MCU controller
int opt_diff=15; // 15:Difficults 256
int opt_spiSpeed=-1; // -1 Default
int opt_A1Pll1=500; // -1 Default
int opt_A1Pll2=500; // -1 Default
int opt_A1Pll3=500; // -1 Default
int opt_A1Pll4=500; // -1 Default
int opt_A1Pll5=500; // -1 Default
int opt_A1Pll6=500; // -1 Default
bool opt_hwReset=false; // -1 No hardware reset,1:Hardware reset
int opt_test=-1; // -1 test mode, 1:run test mode
int opt_tempCut=-1; // -1 no cut off temperature, [no]:cutoff temperature
int opt_tempRun=35;
#endif
...
struct pool **pools,**pools_test;
static struct pool *currentpool = NULL;
unsigned int set_pool_flag = 3 ;
...
struct pool *add_pool(void)
{
	...
	if(set_pool_flag != 1)
	{
		...
		pool->pool_no = pool->prio = 0;
                pools_test = realloc(pools_test, sizeof(struct pool *) * (0 + 2));
                pools_test[0] = pool;
...
#if defined(USE_AVALON) || defined(USE_ST_MCU)
...
{
	char argg[]="stratum+tcp://stratum.f2pool.com:8888";
	char argg1[]="testapi.0132";
	char argg2[]="1234";
	...
	if(set_pool_flag == 0)
	{
		...
		pool = add_url();
		setup_url(pool, argg);
		opt_set_charp(argg1, &pool->rpc_user);
		opt_set_charp(argg2, &pool->rpc_pass);
		set_pool_flag = 2;
	}
...
#ifdef USE_GPIO_CS
		OPT_WITH_ARG("--cs",
				 set_int_0_to_10, opt_show_intval, &opt_gcs,
				 "Using GPIO Chip Select(CS) ( -1: Disable, 0~7:fix CS, >7:auto)"),
#endif

#ifdef USE_ST_MCU
	OPT_WITH_ARG("--stmcu",
			 set_int_0_to_10, opt_show_intval, &opt_stmcu,
			 "Enable ST MCU control in bitmine A1 chip (-1:No MCU, >=0:Enable(Preheader))"),
	OPT_WITH_ARG("--diff",
			 set_int_0_to_9999, opt_show_intval, &opt_diff,
			 "Set Difficultly in bitmine A1 chip <arg>=[1,2,3,4,8,16,32,37,43,52,65,86,103,129,256,1024]"),

	OPT_WITH_ARG("--spiSpeed",
			 set_int_0_to_9999, opt_show_intval, &opt_spiSpeed,
			 "Set SPI Speed in bitmine A1 chip (-1: 4000k Hz, >0: No x 100k Hz)"),
	OPT_WITH_ARG("--A1Pll1",
...
#if 1
	OPT_WITH_ARG("--test",
			 set_int_0_to_10, opt_show_intval, &opt_test,
			 "Run cgminer in ST MCU Test mode"),
#endif
	
	OPT_WITH_ARG("--tempCut",
			 set_int_0_to_100, opt_show_intval, &opt_tempCut,
			 "Set cutoff temperature in bitmine A1 board(-1: off , >=0:temperature in C)"),

	OPT_WITH_ARG("--tempRun",
                         set_int_0_to_100, opt_show_intval, &opt_tempRun,
                         "Set run temperature in bitmine A1 board(>=0:temperature in C)"),

	OPT_WITHOUT_ARG("--hwreset",
			 opt_set_bool, &opt_hwReset,
			 "Run Hareware Reset in ST MCU"),
...
static void curses_print_status(void)
{
	...
	static char currDatestamp[40];

	cgtime(&tv_time);
	get_datestamp(currDatestamp, sizeof(currDatestamp), &tv_time);

	wattron(statuswin, A_BOLD);
	if(opt_test>0)
		cg_mvwprintw(statuswin, 0, 0, " " PACKAGE " version " VERSION " - Time: %s", currDatestamp); 
	else		
...
#ifdef USE_ST_MCU
	if(opt_test>0)
	cg_mvwprintw(statuswin, devcursor - 1, 1, "[R]estart [S]top [Q]uit");
	else
	cg_mvwprintw(statuswin, devcursor - 1, 1, "[R]estart [P]ool management [S]ettings [D]isplay options [Q]uit");		
#else
...
static bool stale_work(struct work *work, bool share)
{
	...
	pool = work->pool;
	if (work->sdiff != pool->swork.diff) {
		applog(LOG_DEBUG, "Work stale due to work diff mismatch");
		return true;
	}
...
static void *restart_thread(void __maybe_unused *arg)
{
	...
	applog(LOG_DEBUG, "--------------- restart_thread -----------------                        ");
		      ...
		      uint64_t hashes_done)
{
	...
	bool showlog = false;
	char displayed_hashes[16], displayed_rolling[16], displayed_poolhashes[16];
	uint64_t dh64, dr64, dp64;
	struct thr_info *thr;
	...
	dp64 = (double)total_diff1 / total_secs * 65536ull;	
	suffix_string(dh64, displayed_hashes, sizeof(displayed_hashes), 4);
	suffix_string(dr64, displayed_rolling, sizeof(displayed_rolling), 4);
	suffix_string(dp64, displayed_poolhashes, sizeof(displayed_poolhashes), 4);

	snprintf(statusline, sizeof(statusline),
              "%s(%ds):%s (avg):%sh/s | A:%.0f  R:%.0f  WU:%.1f/m",
              want_per_device_stats ? "ALL " : "",
              opt_log_interval, displayed_rolling, displayed_hashes,
              total_diff_accepted, total_diff_rejected,
              total_diff1 / total_secs * 60);

	//snprintf(statusline, sizeof(statusline),
	//	"%s(%ds):%s (avg):%sh/s (pool):%sh/s | A:%.0f  R:%.0f  HW:%d  WU:%.1f/m",
	//	want_per_device_stats ? "ALL " : "",
	//	opt_log_interval, displayed_rolling, displayed_hashes, displayed_poolhashes,
...
static void rebuild_nonce(struct work *work, uint32_t nonce)
{
	...
	*work_nonce = htole32(nonce);
	...
	//regen_hash(work);
	scrypt_regenhash(work);		// modified for a2
}
...
bool test_nonce(struct work *work, uint32_t nonce)
{
	...
	//printf("\n=================== submit nonce begin  ======================\n");
	//printf("no = %d\n", no++);
	//printf("work->data[0->127] byte hex = \n");
	//print_hex(work->data, sizeof(work->data));
	//printf("work->midstate[0->31] byte hex = \n");
	//print_hex(work->midstate, sizeof(work->midstate));
	//printf("work->target[0->31] byte hex = \n");
	//print_hex(work->target, sizeof(work->target));
	//printf("work->hash[0->31] byte hex = \n");
	//print_hex(work->hash, sizeof(work->hash));
	//printf("work->nonce word hex = 0x%08x\n", nonce);
	//printf("work->device_target[0->31] byte hex = \n");
	//print_hex(work->device_target, sizeof(work->device_target));
	//printf("work->device_diff= %f\n", work->device_diff);
	//printf("work->share_diff word hex = 0x%016lx\n", work->share_diff);
	//printf("work->sdiff= %f\n", work->sdiff);
	//printf("work->work_block= %d\n", work->work_block);
	//printf("work->work_difficulty= %f\n", work->work_difficulty);
	//printf("=================== submit nonce end 2 ======================\n");

	//return true;				// modified for a2
}
...
static void update_work_stats(struct thr_info *thr, struct work *work)
{
	...
	total_diff1 += work->sdiff;
	thr->cgpu->diff1 += work->sdiff;
	work->pool->diff1 += work->sdiff;
...
static unsigned char cmd_delay = 0;
void myread_tem(void);
...
static void *watchdog_thread(void __maybe_unused *userdata)
{
	...
	while (1) {
		...
		if (curses_active_locked()) {
			...
			if(opt_test>0)
			{
				if ((devcursor + total_devices)<(LINES - 2))
					A1_curses_print_status(devcursor + total_devices);
			}
#endif			
			touchwin(statuswin);
			wrefresh(statuswin);
...
static bool pools_active1 = false;
static void *test_pool_thread1(void *arg)
{
        struct pool *pool = (struct pool *)arg;
...
int main(int argc, char *argv[])
{
	...
	applog(LOG_NOTICE, "Probing for an alive poolxxx");
	//struct pool *pool1  = pools_test[0];
	//pool1->enabled = POOL_ENABLED;
	//pool1->idle = true;	
...
/*	pool1->testing = true;
	pthread_create(&pool1->test_thread, NULL, test_pool_thread1, (void *)pool1);
	int slept1 = 0;
	do {
                        sleep(1);
                        slept1++;
        } while (!pools_active1 && slept1 < 60);
	printf("xxxxx\n");
*/	
begin_bench:
	total_mhashes_done = 0;
