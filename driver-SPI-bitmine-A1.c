...
unsigned long poll_data[6][36]={0};
unsigned char poll_data_start[6][36]={0};
...
unsigned long spi_error_count[6] = {0};
unsigned int asic_no_result[6][36]={0};
unsigned int  hw_big_reset[6]={0};
extern unsigned long bad_ok;
...
struct A1_chain {
	...
	struct work *work_cur[36];
	uint16_t work_start_delay;
	uint16_t asic_wait_delay;
...
void A1_SetA1PLLClock(struct A1_chain *a1,int pllClkIdx)
{
	...
	//printf("pllClkIdx is %d %d\n", chip->reg[8],chip->reg[9]);  // uit voor loggen
...
//wutuhua
...
#endif
	if(( bad_ok > 1)&&(a1->spi_ctx->config.gpio_cs == 0))
		bad_ok--;
		
	if((a1->asic_wait_delay == 0)&&((bad_ok!=1)||(a1->spi_ctx->config.gpio_cs != 0)))
	{
	/* poll queued results */
	while (get_nonce(a1, (uint8_t*)&nonce, &chip_id, &job_id)) {
		
		//if( a1->asic_wait_delay )
		//	break;
		...
		if (!submit_nonce(thr, work, nonce)) {
			...
			//printf(" hw_count_temp[a1->spi_ctx->config.gpio_cs] is %d\n", hw_count_temp[a1->spi_ctx->config.gpio_cs]);
			nonce_cnt++;
			if(nonce_cnt>20)
			{
				printf("wait asic delay......../n");
				spi_send_command(a1, A1_POWER_RESET, 0x00, NULL, 0);
                                cgsleep_us(50);
                                spi_poll_result(a1, A1_POWER_RESET, 0x00, 2);
				//a1->asic_wait_delay = 300;
				//a1->asic_wait_delay = 6000;
				break;
			}	
	...
	if(spi_error_count[a1->spi_ctx->config.gpio_cs] >= 30)//3000
	{
			spi_error_count[a1->spi_ctx->config.gpio_cs] = 0;
			printf(" spi is error..... \n");
			a1->asic_wait_delay = 150;
			//system("sudo reboot");
			//system("sudo ./run.sh");
                        //quit(1, "hw reset........\n");
	...
	if(clear_log[a1->spi_ctx->config.gpio_cs]>=1000)
	{
		...
		#if 0
		switch(a1->spi_ctx->config.gpio_cs){
                	case 0:A1_SetA1PLLClock(a1, A1Pll1);break;
                	case 1:A1_SetA1PLLClock(a1, A1Pll2);break;
               		case 2:A1_SetA1PLLClock(a1, A1Pll3);break;
                	case 3:A1_SetA1PLLClock(a1, A1Pll4);break;
	...
	hw_big_reset[a1->spi_ctx->config.gpio_cs]++;
	if(hw_big_reset[a1->spi_ctx->config.gpio_cs] >= 1000)
	{
		//printf("set vol is try.....\n");
		hw_big_reset[a1->spi_ctx->config.gpio_cs] = 0;
		switch(a1->spi_ctx->config.gpio_cs){
                        case 0:A1_SetA1PLLClock(a1, A1Pll1);break;
                        case 1:A1_SetA1PLLClock(a1, A1Pll2);break;
                        case 2:A1_SetA1PLLClock(a1, A1Pll3);break;
	...
	if(no_result >= 4)
	{
				printf("cs(%d) asic_no_result reset A1_POWER_RESET is try..........\n",a1->spi_ctx->config.gpio_cs);
		 		if(a1->work_start_delay == 1)
	...
	if(hw_time[a1->spi_ctx->config.gpio_cs]>=50)
	{
		...
		int core_i=0,core_j=0,core_all_count = 0;
                ...
                if((core_j>=17)&&(core_reset_time_delay == 0))
                {
		...
		if(hw_count_temp[a1->spi_ctx->config.gpio_cs]>=20)
		{
			//system("sudo ./run.sh");
 			//quit(1, "hw reset........\n");
				printf(" cs(%d)  hw big error is reset .....\n",a1->spi_ctx->config.gpio_cs);
                                if(a1->work_start_delay == 1)
                                {
                                        a1->work_start_delay = 6;
                                }
...
//	 	 printf("hw_count_temp is %d ...........\n",hw_count_temp[a1->spi_ctx->config.gpio_cs]);	
		if(chip_no_work_count[a1->spi_ctx->config.gpio_cs]>=50)//25
		{
			...
			if(bc_reset_count[a1->spi_ctx->config.gpio_cs]>=8)//4
			{
				...
				if(a1->work_start_delay == 1)
			        ...
			        cgsleep_us(50);
			        spi_poll_result(a1, A1_POWER_RESET, 0x00, 2);
				printf("cs(%d) chip_no_work A1_POWER_RESET is try ..... \n",a1->spi_ctx->config.gpio_cs);
		...
		if(core_all_count<1850)
		{
			for(kn=0;kn<20;kn++)
			{
			 	printf(" cs(%d)  chip(%d) cores is %d .....\n",a1->spi_ctx->config.gpio_cs,kn, a1->chips[kn].num_cores);
			}
		}
		#endif
	...
	{	
		...
		if (!cmd_READ_REG(a1, i + 1)) {
			//applog(LOG_ERR, "Failed to read reg from chip(cs%d) %d",a1->spi_ctx->config.gpio_cs,  i);
			// TODO: what to do now?
			spi_error_count[a1->spi_ctx->config.gpio_cs]++;
			continue;
		}
		if(1) {
			a1->chips[i].temp = (a1->spi_rx[14]<<8)|a1->spi_rx[15];
			cgpu->temp = a1->chips[0].temp;
		}
		...
		//if ((a1->spi_rx[11] & 0x01) != 0x01) {
			...
			{		
					...
					//for(n=0;n<2;n++)
					{
						...
						{
						//	printf("chip %d get work is not ok\n",i);
							break;
						}
					}
   		...
   		else if( a1->st_reset_delay > 1)
		{
			...
			if (!set_work(a1, i + 1, a1->work_cur[i]))	
			{
				...
				{
					a1->asic_wait_delay = 0;		
					a1->work_start_delay = 20;
				}
				continue;
			}
			nonce_ranges_processed++;
